...
class zzguardrr_ram extends Module{
  ...
  table.io.addr1    := io.addr
  table.io.addr2    := io.addr
  table.io.data_in1 := io.data_in
  table.io.data_in2 := io.data_in

  when(io.valid_config){
    when(io.funct === 1.U){
      table.io.wen1 := true.B
      table.io.wen2 := false.B
    }
    .elsewhen(io.funct === 2.U){
